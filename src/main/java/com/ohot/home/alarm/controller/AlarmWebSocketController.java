...
public class AlarmWebSocketController {
	...
	public ResponseEntity<String> alarmFromPost(AlarmVO alarmVO){
		...
		//미디어에서 날라올때
		MediaPostVO mediaPostVO = alarmVO.getMediaPostVO();
		if(mediaPostVO !=null && mediaPostVO.getMediaPostNo() !=0 ) {
			notificationVO.setNotiSndrNo(mediaPostVO.getArtGroupNo());
			notificationVO.setNotiOrgNo(mediaPostVO.getMediaPostNo());
			notificationVO.setNotiOrgTb("MEDIA");
			log.info("미디어 실시간 실행"+notificationVO);
		}
